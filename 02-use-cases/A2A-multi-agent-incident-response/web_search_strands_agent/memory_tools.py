"""Memory tools for the Strands-based web search agent.

Converted from OpenAI Agents @function_tool to Strands @tool decorator.
Uses AgentCore Memory for semantic search, conversation history, and persistence.
"""
...
class AgentMemoryTools:
    """Memory tools for Strands agent based on AgentCore Memory."""
    ...
    def create_tools(self) -> list:
        """Create and return all memory-related Strands tools."""

        # Capture instance vars in closure for tool functions
        ...
        def retrieve_monitoring_context(
            query: str, context_type: str = "", top_k: int = 3
        ) -> str:
        ...
        def search_memory_by_namespace(
            ...
            try:
                if namespace_type not in namespaces:
                    ...
                    return f"Invalid namespace type. Available: {available}"
                ...
                return f"No results found in {namespace_type} for: {query}"
            except Exception as e:
...
def create_memory_tools(
    ...
    return memory_tools.create_tools()
